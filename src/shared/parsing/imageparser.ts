import {ImageLayer, ImageType, NodeState} from "../model/enums";
...
export class ImageParser
{
	...
	public ParseJewelAssets(nodeData: any, node: Node): void {
		node.ImagesForState = node.ImagesForState.set(NodeState.Active, Immutable.List(
			[
				this.GetAsset("JewelFrameAllocated").ChangeLayer(ImageLayer.Frame)
			]));

		node.ImagesForState = node.ImagesForState.set(NodeState.Inactive, Immutable.List(
			[
	...
	public ParseMasteryNodeAssets(nodeData: any, node: Node): void {
		...
		node.ImagesForState = node.ImagesForState.set(NodeState.Active, Immutable.List(images));
		node.ImagesForState = node.ImagesForState.set(NodeState.Inactive, Immutable.List(images));
	}
	...
	public ParseClassNodeAssets(classNodeData: any, node: Node): void {
		...
		node.ImagesForState = node.ImagesForState.set(NodeState.Active, Immutable.List(backgroundImages.concat(activeIcon).concat(ascendancyButton)));
		node.ImagesForState = node.ImagesForState.set(NodeState.Inactive, Immutable.List(backgroundImages.concat(inactiveIcon)));
	}
